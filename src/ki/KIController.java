...
public class KIController {
	...
	public void run(){
		...
		for (Player p1:getGame().getPlayers()) {
			allFields=p1.getFields();
			for(Vertex v1:allFields) {
				...
				if (v1.getField().getResources().get(Resources.FOOD) < 0 && r.nextInt(100) < 50 && v1.getField().getResources().get(POPULATION) > 1) {
					...
					p1.getNotifications().add(new RebellionNotification(!p1.equals(v1.getField().getPlayer()), v1));
					for (Notification n : p1.getNotifications()) {
						if (n instanceof AttackNotification) {
							if (((AttackNotification) n).getOpponent().equals(rebelPlayer)) {
								p1.getNotifications().remove(n);
							}
						}
					}
				}

				if (p1 instanceof KIFraction) {
					...
					if (!current.isFraction()) {
						...
						if (r.nextInt(100) <= current.getDevelopingChance()) {
							...
							if (!options.isEmpty()) {
								...
								current.setDevelopingChance(r.nextInt(5*p1.getFields().size()));
							}
						}
